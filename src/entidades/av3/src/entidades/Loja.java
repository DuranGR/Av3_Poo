...
public class Loja {
	...
	public Produto retornaProduto(int codigo) {
		...
		for (Produto produto : produtos) {
			if (codigo == produto.getCodigo()) {
				// Vai retornar o produto
				return produto;
			}

		}
		return null; // Significa produto nao encontrado
	}
	...
	public boolean comprarItem(Produto prod) {
		double valorTotal = prod.getPreco() * prod.getQuantidade()*imposto;
		...
		dinheiro -= valorTotal;
		produtos.add(prod); // Adiciona o produto comprado à lista
	...
	public boolean comprarItem(int codigoProduto, int quantidade) {
		...
		double valorTotal = quantidade * produtos.get(indexProduto).getPreco() * imposto;
		// Verifica se o produto foi encontrado e se ha dinheiro suficiente
		if (indexProduto != -1 && valorTotal <= dinheiro) {

			// Atualiza a quantidade no estoque
			produtos.get(indexProduto).setQuantidade(produtos.get(indexProduto).getQuantidade() + quantidade);

			dinheiro -= valorTotal; // Atualiza o dinheiro apos a compra
