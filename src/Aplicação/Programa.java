...
import serviços.ImpostoAcessorioBr;
import serviços.ImpostoConsoleBr;
import serviços.ImpostoJogosBr;
import serviços.ServicoImposto;
...
public class Programa {
	...
	public static void main(String[] args) {
		...
		ServicoImposto servicoImpostoJogo = new ServicoImposto(new ImpostoJogosBr());
		ServicoImposto servicoImpostoConsole = new ServicoImposto(new ImpostoConsoleBr());
		;
		ServicoImposto servicoImpostoAcessorio = new ServicoImposto(new ImpostoAcessorioBr());
		;
				...
				if (opcaoEscolhida == 1) {
					...
					if (opcaoEscolhida == 1) {
						...
						if (loja.encontrarProduto(codigoAux) != -1) {
							...
							produtoAux = loja.retornaProduto(codigoAux);
							
							// Mude o tipo de imposto usado com base na instância do produto
							if (produtoAux instanceof Jogos) { 
								servicoImpostoJogo.processarImposto(loja);
							} else if (produtoAux instanceof Console) {
								servicoImpostoConsole.processarImposto(loja);
							} else if (produtoAux instanceof Acessorios) {
								servicoImpostoAcessorio.processarImposto(loja);
							}
